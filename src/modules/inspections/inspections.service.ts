...
import { InspectionItemMedia } from './entities';
dotenv.config();
...
export class InspectionsService {
  ...
  const draftInspections = inspections.filter(
    (inspection) => inspection.status === InspectionStatus.DRAFT,
  ).length;
  ...
  //   const expiresIn = parseInt(process.env.FILE_EXPIRES_SECONDS || '172800');
  ...
  try {
    ...
    if (files?.length) {
      ...
      media = await this.mediaRepo.save(mediaRecords);
    }
